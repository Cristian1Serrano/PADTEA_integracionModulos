...
$(document).ready(function() { //
    ...
    $(".enviar-btn").keypress(function(event) {
      ...
      if ( event.which == 13 ) {
        var getpID =  $(this).parent().attr('id').replace('record-',''); // obtener ID de publicacion 
...
$CantidadMostrar=5;
	...
	$TotalReg       =mysqli_query("SELECT * FROM publicaciones");
	...
	while ($lista=mysqli_fetch_array($consulta)) {
		...
		$usuariob = mysqli_query("SELECT * FROM usuarios WHERE id_use = '$userid'");
    $use = mysqli_fetch_array($usuariob);

    $fotos = mysqli_query("SELECT * FROM fotos WHERE publicacion = '$lista[id_pub]'");
	...
	<!-- PUBLICACIONES -->
          ...
          <div class="box box-widget">
            ...
            <div class="box-body">
              ...
              <?php
              // Si la publicacion tiene imagen, la mostramos 
              if($lista['imagen'] != 0)
              {
              ?>
              ...
              <br><br>
              <?php 
              $numcomen = mysqli_num_rows(mysqli_query("SELECT * FROM comentarios WHERE publicacion = '".$lista['id_pub']."'"));
              ?>
              <!-- boton social -->
            <ul class="list-inline">

              <?php
              $query = mysqli_query("SELECT * FROM likes WHERE post = '".$lista['id_pub']."' AND usuario = ".$_SESSION['id']."");
            ...
            <?php 
            $comentarios = mysqli_query("SELECT * FROM comentarios WHERE publicacion = '".$lista['id_pub']."' ORDER BY id_com desc LIMIT 2");
            while($com=mysqli_fetch_array($comentarios)){
              $usuarioc = mysqli_query("SELECT * FROM usuarios WHERE id_use = '".$com['usuario']."'");

...
<?php
  ...
  $infouser = mysqli_query("SELECT * FROM usuarios WHERE id_use = '$id'");
  $use = mysqli_fetch_array($infouser);

  $amigos = mysqli_query("SELECT * FROM amigos WHERE de = '$id' AND para = '".$_SESSION['id']."' OR de = '".$_SESSION['id']."' AND para = '$id'");
...
<head>
  ...
  <title><?php echo $use['nombre']; ?> | PADTEA</title>
          ...
          <div class="box box-primary">
            <div class="box-body box-profile">
              ...
              if(isset($_POST['seguir'])) {
                $add = mysqli_query("INSERT INTO amigos (de,para,fecha,estado) values ('".$_SESSION['id']."','$id',now(),'0')");
              ...
              if(isset($_POST['seguirdirecto'])) {
                $add = mysqli_query("INSERT INTO amigos (de,para,fecha,estado) values ('".$_SESSION['id']."','$id',now(),'1')");
                if($add) {echo '<script>window.location="perfil.php?id='.$id.'"</script>';}
              }
              ?>
              ...
              if(isset($_POST['dejarseguir'])) {
                $add = mysqli_query("DELETE FROM amigos WHERE de = '$id' AND para = '".$_SESSION['id']."' OR de = '".$_SESSION['id']."' AND para = '$id'");
                if($add) {echo '<script>window.location="perfil.php?id='.$id.'"</script>';}
              }
              ?>

...
<div class="register-box">
  ...
  <div class="register-box-body">
    <p class="login-box-msg">Regístrate en PADTEA</p>
    ...
    <?php
    // si se pulsa el boton de registrar se ejecuta el siguiente codigo
    if(isset($_POST['registrar'])) {
      // se crea una variable para cada campo del formulario
      $nombre = mysqli_real_escape_string($_POST['nombre']);
      $email = mysqli_real_escape_string($_POST['email']);
      $usuario = mysqli_real_escape_string($_POST['usuario']);
      $contrasena = mysqli_real_escape_string(md5($_POST['contrasena']));
      $repcontrasena = mysqli_real_escape_string(md5($_POST['repcontrasena']));
      // se crea una variable para comprobar si el usuario ya existe
      $comprobarusuario = mysqli_num_rows(mysqli_query("SELECT usuario FROM usuarios WHERE usuario = '$usuario'"));

      $comprobaremail = mysqli_num_rows(mysqli_query("SELECT email FROM usuarios WHERE email = '$email'"));
     ...
     <?php } else {
        ...
        <?php } else {
          ...
          <?php } else {
            ...
            // si todo esta bien se insertan los datos en la base de datos 
            $insertar = mysqli_query("INSERT INTO usuarios (nombre,email,usuario,contrasena,fecha_reg) values ('$nombre','$email','$usuario','$contrasena',now())");
